...
const EMPTY = {
  name: '',
  officialName: '',
  type: 'UNIDADE' as 'UNIDADE' | 'CLIENTE',
  matchColigada: '',
  matchFilial: '',
  matchClientePattern: '',
  latitude: '',
...
export default function LocaisPage() {
  ...
  async function importLocations(rows: Record<string, string>[]): Promise<string> {
    ...
    for (const row of rows) {
      ...
      const type = tipoRaw.startsWith('CLIENTE') ? 'CLIENTE' : 'UNIDADE'
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Unidades do grupo (casam por coligada/filial) e clientes (casam por trecho do nome na venda).
        </p>
      </div>
      ...
      <form onSubmit={save} className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-6">
          ...
          <div>
            ...
            <select
              value={form.type}
              onChange={(e) => setForm({ ...form, type: e.target.value as 'UNIDADE' | 'CLIENTE' })}
              className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
            >
              <option value="UNIDADE">Unidade do grupo</option>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
            ...
            <ExcelButtons
              ...
              exportRows={() =>
                filteredLocations.map((l) => ({
                  ...
                  Tipo: l.type === 'UNIDADE' ? 'Unidade' : 'Cliente',
        ...
        <p className="border-b border-slate-100 px-4 py-2 text-[11px] text-slate-500">
          ...
          <strong>Tipo</strong> (Unidade/Cliente), <strong>Coligada</strong>, <strong>Filial</strong>,{' '}
          <strong>Trecho do nome</strong>, <strong>Latitude</strong>, <strong>Longitude</strong> e{' '}
          <strong>Raio (m)</strong>. Nome já cadastrado é atualizado; nome novo é criado.
        </p>
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2">Trecho do nome</th>
            ...
            {filteredLocations.map((l) => (
              <tr key={l.id} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-xs ${l.type === 'UNIDADE' ? 'bg-emerald-100 text-emerald-800' : 'bg-cyan-100 text-cyan-800'}`}
                  >
                    {l.type === 'UNIDADE' ? 'Unidade' : 'Cliente'}
                  </span>
                </td>
