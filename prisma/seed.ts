...
// BUG REAL corrigido 2026-09-22 (achado do usuário: "conferir as notas do
// cliente DAVID MADEIRA IMUNIZADA no mes de agosto, nenhum esta calculando
// a bonificacao") — confirmado ao vivo no Oracle: o cliente C00004726 não
// tem NENHUMA tabela de preço cadastrada no próprio CODCFO, mas o
// distribuidor dele (FCFOCOMPL.DISTRIBUIDOR = C00003288, EXTRA) TEM uma
// "DISTRIBUIDOR 7%" válida cobrindo exatamente as datas de venda de agosto
// — a subquery só olhava `TTABPRECOCFO.CODCFO = TMOV.CODCFO` (o cliente),
// nunca o distribuidor, então nunca achava a tabela e caía sempre no
// PRECOUNITARIO (bonificação zerada). Já existem casos reais dos dois
// padrões (cliente com tabela própria, ex. MADEIREIRA PONTUAL C00003756; e
// cliente sem tabela própria, só o distribuidor, ex. este caso) — a
// subquery agora tenta os dois CODCFO (`IN (TMOV.CODCFO,
// FCFOCOMPL.DISTRIBUIDOR)`), priorizando o do cliente quando os dois
// existirem (ORDER BY a prioridade, ROWNUM=1 depois de ordenado).
...
END BONIFICACAO,
CASE
  WHEN (SELECT PRECO FROM (
          SELECT TTABPRECOPRD.PRECO,
                 CASE WHEN TTABPRECOCFO.CODCFO = TMOV.CODCFO THEN 0 ELSE 1 END PRIORIDADE
          ...
          AND   TTABPRECOCFO.CODCFO IN (TMOV.CODCFO, FCFOCOMPL.DISTRIBUIDOR)
